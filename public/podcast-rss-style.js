/* Based on https://www.rss.style/ — customised for podcast feeds */
console.log(`INFO: processing podcast feed from ${window.location.href}`);
document.onreadystatechange = async function () {
    if (document.readyState === "complete") {
        ...
        pRss.appendChild(document.createTextNode("This is the podcast "));
        const rssLink = document.createElementNS(NS, "a");
        rssLink.setAttribute("href", "https://www.rss.style/what-is-a-feed.html");
        rssLink.textContent = "feed";
        pRss.appendChild(rssLink);
        pRss.appendChild(document.createTextNode(` for ${title.textContent}.`));
        if (homeLink) {
            var btnHomeLink = document.createElementNS(NS, "button");
            btnHomeLink.setAttribute("onclick", `window.location.href = \"${homeLink}\";`);
        ...
        for (let item of items) {
            ...
            const itemPubDateRaw = item.querySelector("pubDate").textContent;
            const itemPubDate = new Date(itemPubDateRaw).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit" });
            ...
            if (enclosureUrl) {
                const audio = document.createElementNS(NS, "audio");
                audio.setAttribute("controls", "");
                audio.setAttribute("src", enclosureUrl);
                audio.setAttribute("style", "width:100%;margin-top:0.75em;");
                details.appendChild(audio);
            } else {
                const noAudio = document.createElementNS(NS, "p");
                noAudio.setAttribute("style", "color:#888;font-style:italic;margin-top:0.5em;");
                noAudio.textContent = "No audio file available for this episode.";
                details.appendChild(noAudio);
            }
        ...
        pCount.textContent = `${items.length} episode${items.length === 1 ? "" : "s"}.`;
