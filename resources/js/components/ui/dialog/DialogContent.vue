...
<template>
  <DialogPortal>
    ...
    <DialogContent
      ...
      :class="
        cn(
          'bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-dvw max-h-dvh overflow-y-auto translate-x-[-50%] translate-y-[-50%] gap-4 rounded-none border-3 border-neo-dark p-3 sm:p-6 duration-200 sm:max-w-lg',          props.class,
        )"
    >
      <slot />
